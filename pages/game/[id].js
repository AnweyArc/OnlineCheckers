...
const GamePage = () => {
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center bg-slate-900 p-4 overflow-hidden">
      ...
      <div className="relative z-20 w-full max-w-2xl bg-gradient-to-br from-purple-100 via-sky-100 to-pink-50 p-[2px] rounded-2xl shadow-2xl">
        <div className="bg-slate-900/95 backdrop-blur-sm rounded-2xl px-4 md:px-6 py-3 md:py-4 mb-4 md:mb-6">
          <div className="flex flex-col sm:flex-row justify-between items-center">
            <h1 className="text-lg md:text-xl lg:text-2xl font-bold text-white mb-2 sm:mb-0 text-center">
              Game ID: <span className="text-sky-300">{gameId?.slice(0, 6)}...</span>
            </h1>
            <button
  ...
  <p className="text-base md:text-lg font-semibold text-gray-100">
        ...
        <div className="px-4 pb-4">{renderBoard()}</div>
        ...
        {(playerRole === 'r' || playerRole === 'b') && (
          <>
            {game?.status === 'won' || game?.status === 'forfeited' ? (
              <button
                onClick={() => router.push('/')}
                className="mt-4 md:mt-6 mx-4 px-4 py-1.5 md:px-6 md:py-2 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white rounded-lg font-medium transition-all transform hover:scale-[1.02] flex items-center gap-2 text-sm md:text-base"
              >
