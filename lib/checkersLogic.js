...
export const applyMove = (board, srcRow, srcCol, destRow, destCol) => {
  const validMoves = getValidMoves(board, board[srcRow][srcCol].toLowerCase());
  const matchingMove = validMoves.find(m => m.from[0] === srcRow && m.from[1] === srcCol && m.to[0] === destRow && m.to[1] === destCol);
  ...
  return applyMoveWithCaptures(board, [srcRow, srcCol], [destRow, destCol], captures);
};
