...
// ✅ Forced capture removed — returns all legal moves (not just jumps)
export const getValidMoves = (board, player) => {
  ...
  // ✅ Return all valid moves — capture not forced
  return [...moves, ...jumps];
};
...
export const checkWinCondition = (board) => {
  ...
  const redHasMoves = redPieces.some(([r, c]) =>
    getValidMoves(board, 'r').length > 0
  );
